    ...
    <div className="h-screen w-full flex flex-col bg-background text-foreground overflow-hidden">
      <header className="h-14 border-b border-border bg-card shrink-0 flex items-center px-4 justify-between gap-3">
        <div className="flex items-center gap-3 min-w-0">
          ...
          <div className="font-mono text-sm text-muted-foreground shrink-0">ID_{session.id.toString().padStart(4, "0")}</div>
          <Badge variant="outline" className={`font-mono text-[10px] rounded-none uppercase border shrink-0 ${session.status === "done" ? "border-emerald-500/40 text-emerald-400 bg-emerald-500/10" : session.status === "failed" ? "border-red-500/40 text-red-400 bg-red-500/10" : session.status === "cancelled" ? "border-gray-500/40 text-gray-400 bg-gray-500/10" : "border-amber-500/40 text-amber-400 bg-amber-500/10"}`}>{session.status}</Badge>
      ...
      <div className="flex-1 flex overflow-hidden">
        <div className="w-1/4 min-w-[240px] max-w-[320px] border-r border-border flex flex-col bg-card">
          ...
          <div className="flex-1 bg-background overflow-hidden flex flex-col">
            {selectedFile ? <><div className="h-8 shrink-0 bg-muted/20 border-b border-border flex items-center px-3 font-mono text-[10px] text-muted-foreground justify-between gap-2"><span className="truncate">{selectedFile.name}</span><div className="flex items-center gap-1 shrink-0"><span className="text-muted-foreground/50">{selectedFile.language}</span>{editingFileId === selectedFile.id ? <><button onClick={handleSaveEdit} disabled={updateFile.isPending} className="ml-1 p-0.5 text-emerald-400 hover:text-emerald-300 transition-colors" title="Save changes"><Save className="w-3 h-3" /></button><button onClick={() => setEditingFileId(null)} className="p-0.5 text-muted-foreground hover:text-foreground transition-colors" title="Cancel edit"><XIcon className="w-3 h-3" /></button></> : <button onClick={() => { setEditingFileId(selectedFile.id); setEditContent(selectedFile.content); }} className="ml-1 p-0.5 text-muted-foreground hover:text-primary transition-colors" title="Edit file"><Pencil className="w-3 h-3" /></button>}</div></div>{editingFileId === selectedFile.id ? <textarea value={editContent} onChange={(e) => setEditContent(e.target.value)} className="flex-1 w-full p-3 font-mono text-[11px] bg-background text-foreground resize-none focus:outline-none border-0 leading-relaxed" spellCheck={false} /> : <ScrollArea className="flex-1"><pre className="p-3 text-[11px] font-mono leading-relaxed text-foreground/90 overflow-x-auto whitespace-pre-wrap break-all"><code>{selectedFile.content}</code></pre></ScrollArea>}</> : <div className="flex-1 flex items-center justify-center font-mono text-xs text-muted-foreground">SELECT_FILE</div>}
          </div>
        </div>
        ...
        <div className="flex-1 border-r border-border flex flex-col bg-background min-w-0">
          ...
          <ScrollArea className="flex-1 p-4">
            <div className="flex flex-col gap-4">
              {filteredEvents.length === 0 && <div className="text-center p-8 font-mono text-xs text-muted-foreground">{events.length === 0 ? (isActive ? "INITIALIZING..." : "NO_EVENTS") : "NO_MATCHES"}</div>}
              {filteredEvents.map((event) => <div key={event.id} className="flex gap-3"><div className="shrink-0 pt-1">{event.type === "thought" && <div className="w-1.5 h-1.5 rounded-full bg-blue-400/60" />}{event.type === "plan" && <div className="w-1.5 h-1.5 rounded-full bg-violet-400/60" />}{event.type === "code" && <div className="w-1.5 h-1.5 rounded-full bg-amber-400/80" />}{event.type === "test" && <div className="w-1.5 h-1.5 rounded-full bg-sky-400/60" />}{event.type === "error" && <div className="w-1.5 h-1.5 rounded-full bg-red-500" />}{event.type === "success" && <div className="w-1.5 h-1.5 rounded-full bg-emerald-500" />}{event.type === "git" && <div className="w-1.5 h-1.5 rounded-full bg-gray-500" />}</div><div className="flex-1 min-w-0"><div className="flex items-center gap-2 mb-0.5"><span className={`font-mono text-[10px] uppercase font-bold ${event.type === "error" ? "text-red-400" : ""} ${event.type === "success" ? "text-emerald-400" : ""} ${event.type === "code" ? "text-amber-400" : ""} ${event.type === "plan" ? "text-violet-400" : ""} ${event.type === "thought" ? "text-blue-400" : ""} ${["test", "git"].includes(event.type) ? "text-muted-foreground" : ""}`}>[{event.type}]</span><span className="font-mono text-[9px] text-muted-foreground/50">{safeFormat(event.createdAt, "HH:mm:ss")}</span>{event.iteration > 0 && <span className="font-mono text-[9px] text-muted-foreground/40 border border-border px-1">IT:{event.iteration}</span>}</div><div className={`font-mono text-[11px] whitespace-pre-wrap break-words leading-relaxed ${event.type === "error" ? "text-red-400/90 bg-red-500/10 p-2 border border-red-500/20" : event.type === "success" ? "text-emerald-400/90 bg-emerald-500/10 p-2 border border-emerald-500/20" : event.type === "code" ? "text-foreground/80 bg-muted/30 p-2 border border-border" : "text-foreground/75"}`}>{event.content}</div></div></div>)}
          ...
          <Tabs defaultValue="tests" className="flex-1 flex flex-col overflow-hidden">
            <div className="h-10 border-b border-border bg-background px-2 flex items-center shrink-0">
              <TabsList className="bg-transparent h-8 p-0 gap-4">
                <TabsTrigger value="tests" className="font-mono text-[10px] uppercase data-[state=active]:bg-transparent data-[state=active]:shadow-none data-[state=active]:text-primary border-b-2 border-transparent data-[state=active]:border-primary rounded-none px-2 h-full">Telemetry</TabsTrigger>
                <TabsTrigger value="git" className="font-mono text-[10px] uppercase data-[state=active]:bg-transparent data-[state=active]:shadow-none data-[state=active]:text-primary border-b-2 border-transparent data-[state=active]:border-primary rounded-none px-2 h-full">VCS</TabsTrigger>
                <TabsTrigger value="history" className="font-mono text-[10px] uppercase data-[state=active]:bg-transparent data-[state=active]:shadow-none data-[state=active]:text-primary border-b-2 border-transparent data-[state=active]:border-primary rounded-none px-2 h-full">History</TabsTrigger>
              </TabsList>
            </div>
                ...
                <ScrollArea className="flex-1">
                  <div className="p-3">
                    <div className="font-mono text-xs flex flex-col gap-4">
                      {!gitStatus ? <div className="text-center p-6 font-mono text-[10px] text-muted-foreground">VCS_UNAVAILABLE</div> : <>
                        <div>
                          <div className="text-[10px] text-muted-foreground uppercase mb-1">Branch</div>
                          <div className="flex items-center gap-2 text-primary bg-primary/10 px-2 py-1 border border-primary/20"><GitBranch className="w-3 h-3" />{gitStatus.branch}</div>
                        </div>
                        <div>
                          <div className="text-[10px] text-muted-foreground uppercase mb-1">Staged ({gitStatus.staged?.length || 0})</div>
                          {gitStatus.staged?.length ? gitStatus.staged.map((f) => <div key={f} className="text-emerald-400 mb-0.5 truncate pl-2 border-l border-emerald-500/30 text-[10px]">A {f}</div>) : <div className="text-[10px] text-muted-foreground/50 pl-2">clean</div>}
                        </div>
                        <div>
                          <div className="text-[10px] text-muted-foreground uppercase mb-1">Modified ({gitStatus.modified?.length || 0})</div>
                          {gitStatus.modified?.length ? gitStatus.modified.map((f) => <div key={f} className="text-amber-400 mb-0.5 truncate pl-2 border-l border-amber-500/30 text-[10px]">M {f}</div>) : <div className="text-[10px] text-muted-foreground/50 pl-2">clean</div>}
                        </div>
                        <div>
                          <div className="text-[10px] text-muted-foreground uppercase mb-1">Untracked ({gitStatus.untracked?.length || 0})</div>
                          {gitStatus.untracked?.length ? gitStatus.untracked.map((f) => <div key={f} className="text-muted-foreground mb-0.5 truncate pl-2 border-l border-muted-foreground/30 text-[10px]">? {f}</div>) : <div className="text-[10px] text-muted-foreground/50 pl-2">none</div>}
                        </div>
                      </>}
                    </div>
                  </div>
